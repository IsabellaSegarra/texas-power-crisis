...
```
Now I will find regions further than 200m buffer. 
```{r}
...
# Join blackout highways and houses #UPDATE HERE
buildings_blackout <- st_join(houses, blackout_highways, join = st_intersects, left = TRUE)
# NEW WORKFLOW 
buildings_blackout_number <- houses[blackout_highways, op = st_intersects]
# number of buildings 
homes_affected <- nrow(buildings_blackout_number)

# Update column name to blackout
buildings_blackout<- buildings_blackout %>% rename(blackout = VNP46A1.A2021038.h08v05.001.2021039064328.tif)



print(paste("The total number of affected homes are", homes_affected, "homes."))
...
residential_estimate %>% kable(col.names = c("Type of building", "Number of blackouts"), caption = "Table 1. Estimate of residential homes that experinced blackouts") %>%
...
# Save the number of affected homes 
non_affected_homes <- nrow(buildings_no_blackout)

print(paste("The total number of not affected homes are", non_affected_homes, "homes."))

# non-affected buildings
buildings_no_blackout <- houses[blackout_highways, op = st_disjoint]
...
tm_shape(buildings_blackout) + 
  ...
  #tm_basemap("CartoDB.PositronNoLabels") + # basemap won't work
  tm_title("Houses in Houston Affected by the 2021 Power Crisis", fontface = "bold") +
  tm_layout(bg.color = "white", legend.outside = TRUE) +
  tm_add_legend(type = "polygons", labels = "Experienced blackout", fill = "#2f0873", title = "Houston homes") +
  tm_compass(size = 0.8) +
...
# Match extent of socieoco to buildings blackout 
# creat bbounding box of buildings blackout
bbox_bl <- st_as_sfc(st_bbox(buildings_blackout))
# match soc df to bounding box 
socieoco <- st_intersection(socieoco, bbox_bl)
...
soc_blackout <- st_join(x = socieoco, y = buildings_blackout, join = st_intersects)
...
soc_no_blackout <- st_join(socieoco, buildings_no_blackout, join = st_intersects)
...
In this section I mapped the census tracts that lost power. 
```{r}

# define palette
my_palette <- c("#8ecae6", "#219ebc", "#023047", "#ffb703", "#fb8500")
      
tm_shape(soc_blackout) + 
  tm_polygons(fill = "income_blackout", 
              fill.legend = tm_legend(title = "Median Income"), 
              fill.scale = tm_scale(breaks = c(0, 50000, 100000, 150000, 20000, 300000), 
                                    values = my_palette)) +
  tm_title(text = "Median Income of Households Affected by Blackouts") +
  tm_compass() +
  tm_scale_bar() +
  tm_basemap()
...
## Discussion
Based on the results of this investigation, it is clear that the 2021 Texas power crisis had widespread impacts across the state and even more pronounced effects in the city of Houston. From observing changes in night lights, I saw that large regions of Houston experienced major blackouts spanning more than 10km. Looking at Census tract data, more houses with median incomes below $20,000 per year experienced blackouts compared to those that did not. As with many dense urban areas, much of the variability in the data is not captured. Because of this, it is safe to assume that the effects on Texan residents—especially in cities like Houston—are likely underrepresented.
