...
```
Since the CRS of 'socioeco_geom' is inconsisent with the CRSs of the rest of the data, I will transform this data to match the rest of the data. 
...
In this section, I mapped the night lights for February 2nd and 16th. 
```{r}
# Create bounding box for Houston area 
bbox_houston <- c(xmin = -96.5, xmax = -94.5, ymin = 29, ymax = 30.5)
```
...
# this will produce a conditional and any object that is an NA is more than 200 
# True - more than 200, False - less than 200 
...
# st_make_valid (check with sstisvalid first)

# vector difference mask 

# crop with st_bbox (convert w/ st_as_sf) or with st_points (more tedious) 
...
# Create conditional where Trues are regions that experienced a blackout and 
blackout <- (nl_16 - nl_07) > 200

# Reassign FALSE as NA
...
# Crop your sf object to that bbox
blackout <- st_crop(blackout, bbox_houston)

# Update CRS

#blackout <- st_bbox(c(xmin = -96.5, xmax = -94.5, ymax = 30.5, ymin = 29),crs = st_crs(blackout))

print(class(blackout))
...
## Highways
```{r}
# identity areas within 200 m of all hgiways (use st_buffer)
