...
tm_shape(buildings_blackout) + 
  tm_polygons(col = "#2f0873", title = "Houston Homes") +
...
soc_blackout_no_geom <- soc_blackout %>% 
  mutate(GEOID = as.numeric(GEOID)) %>%  # overwrite GEOID
  st_drop_geometry()

soc_no_blackout_no_geom <-soc_no_blackout %>%  
...
soc_join <- st_join(soc_blackout_no_geom, soc_no_blackout_no_geom, by = "GEOID", left = TRUE)

soc_join <- soc_blackout_no_geom %>%
  left_join(soc_no_blackout_no_geom, by = "GEOID")
...
Below are attemps at mapping the census tracts that lost power. Tmap() is struggling to map this large of a dataset. I plan to continue to troubleshoot this error. 
```{r}
#| output: false 
...
class(soc_blackout)
nrow(soc_blackout)
st_geometry(soc_blackout)

# Simplify the data
#soc_blackout_simple <- st_simplify(soc_blackout, dTolerance = 100)  
# Filter for only relevant columns (fail to run)
